...
echo form_open("/calc/tostepndo/".$dt[0]->NPK_ID);
echo form_hidden("npk_id", $dt[0]->NPK_ID);
echo form_hidden("step", $step_n);
echo "<br><input type='submit' name='prev' value='<< Prev Step' />";
...
echo "<tr><td valign='top'>";
	echo "<u>Calculation - ".($step_n-1)."</u>";
	echo "<table>";
	echo "<tbody>";
	foreach($proc as $k=>$r) {
		echo "<tr><td valign='top'><button class='btnfml'>[".$r->CF_NAME."]</button><br>~ ".$r->STR_FORMULA."</td><td valign='top'>= </td><td align='right' valign='top'>".number_format($r->CF_NOM, 2)."</td><td>&nbsp;</td></tr>";
	}
	echo "</tbody>";
	echo "</table>";
...
echo "</td><td valign='top'>";
	echo "<u><b>Calculation - ".$step_n."</b></u>";
	echo "<table>";
	echo "<tbody>";
	$i = 1;//print_r($proc_exist);
	if(count($proc_exist)>0) {
		foreach($proc_exist as $k=> $v) {
			echo "<tr><td valign='top' nowrap>".form_textarea(array("name"=>"str_formula_exist[".$v->CF_ID."]", "cols"=>30, "rows"=>3), $v->STR_FORMULA)."</td><td valign='top'>=</td>";
			echo "<td valign='top' align='right'>".number_format($v->CF_NOM, 2)."</td>";
			echo "<td valign='top'><a href='".site_url("/calc/setasfee/".$dt[0]->NPK_ID."/".$step_n."/".$v->CF_ID)."' class='setasfee'><img src='".image_asset_url("fee.jpg")."' width=18 /></a>";
...
echo "</td></tr>";
echo "</table>";
echo "<br><hr><input type='submit' name='prev' value='<< Prev Step' />";
echo "&nbsp;&nbsp;<input type='submit' name='submit' value='Submit Formula' />";
echo "&nbsp;&nbsp;<input type='submit' name='calculate' value='Calculate' />";
if(count($proc_exist)>0) { echo "&nbsp;&nbsp;<input type='submit' name='next' value='Next Step >>' />"; }
echo form_close();

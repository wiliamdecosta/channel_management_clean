...
<script>
    ...
    function modal_lov_cust_pgl_show(the_id_field, the_code_field) {
        modal_lov_cust_pgl_set_field_value(the_id_field, the_code_field);
        $("#modal_lov_cust_pgl").modal({backdrop: 'static'});
        modal_lov_cust_pgl_prepare_table();
    }


    function modal_lov_cust_pgl_set_field_value(the_id_field, the_code_field) {
         $("#modal_lov_cust_pgl_id_val").val(the_id_field);
         $("#modal_lov_cust_pgl_code_val").val(the_code_field);
    }

    function modal_lov_cust_pgl_set_value(the_id_val, the_code_val) {
        ...
        $("#modal_lov_cust_pgl_grid_selection").bootgrid({
    	     formatters: {
                "opt-edit" : function(col, row) {
                    return '<a href="#'+ $("#modal_lov_cust_pgl_code_val").val() +'" title="Set Value" onclick="modal_lov_cust_pgl_set_value(\''+ row.PGL_ID +'\', \''+ row.PGL_NAME +'\')" class="blue"><i class="ace-icon fa 	fa-pencil-square-o bigger-130"></i></a>';
                }
             },
    	     rowCount:[5,10],
    		 ajax: true,

...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        jQuery("#grid-table").jqGrid({
            url: '<?php echo site_url('parameter/gridUserAttributeType');?>',
            datatype: "json",
            mtype: "POST",
            colModel: [
                {
                    label: 'ID',
                    name: 'P_USER_ATTRIBUTE_TYPE_ID',
                    key: true,
                    width: 35,
                    sorttype: 'number',
                    sortable: true,
            ...
            viewrecords: true,
            rowList: [5, 10, 20],
            sortname: 'CODE', // default sorting ID
            ...
            onSelectRow: function (rowid) {
                var celValue = $('#grid-table').jqGrid('getCell', rowid, 'CODE');
                var grid_id = jQuery("#jqGridDetails");
                if (rowid != null) {
                    grid_id.jqGrid('setGridParam', {
                        url: "<?php echo site_url('parameter/gridUserAttributeList');?>/" + rowid,
                        datatype: 'json',
                        postData: {parent_id: rowid},
                        userData: {row: rowid}
                    });
                    grid_id.jqGrid('setCaption', 'ATTRIBUTE TYPE LIST :: ' + celValue);
...
//            multipleSearch: true,
    ...
    $("#jqGridDetails").jqGrid({
        ...
        rownumWidth: 35, // the width of the row numbers columns
        viewrecords: true,
        sortname: 'NAME ', // default sorting ID
        caption: 'Daftar Attribut List',
        sortorder: 'asc',
        ...
        editurl: '<?php echo site_url('parameter/crud_user_attribute_list');?>'
    });

...
class Loaddata extends CI_Controller {
    ...
    public function databulanOthers() {
        $title = $_POST['title'];
        //BreadCrumb
        ...
        $this->load->view('loaddata/data_bulanan_others',$data);
    }

    public function grid_databulanan_others() {
       ...
       // $req_param['value'] = array(1);
        ...
        $req_param['where'] = array('P_BATCH_TYPE_ID' => 10);
    ...
    public function createBatchNOM(){
        ...
        $i_pgl_id = $this->input->post('i_pgl_id');
        $period = implode('#',$tmpPeriod);
        $result['data']= $this->loadData->create_batch_nom($period,$username,$i_pgl_id);
        json_encode($result);
    }
    ...
    public function loadflagPaymentOthers(){
        $title = "Flag Payment(PLG)";
        //BreadCrumb
        $bc = array($this->head,$title);
        $this->breadcrumb = getBreadcrumb($bc);
        ...
        $this->load->view('loaddata/load_flagpayment_others',$data);
    }

    public function grid_load_Payment_others(){
        ...
        $req_param['where'] = array('P_BATCH_TYPE_ID' => 11);
	...
	public function loadflagPayment(){
		$title = "Load Flag Pembayaran";
        //BreadCrumb
        $bc = array($this->head,$title);
        $this->breadcrumb = getBreadcrumb($bc);
        ...
        $this->load->view('loaddata/load_flagpayment',$data);
	}
	
	public function grid_load_Payment(){
        ...
        $req_param['where'] = array('P_BATCH_TYPE_ID' => 3);
    ...
    public function grid_dataNOM() {
        ...
        $req_param['where'] = array('P_BATCH_TYPE_ID' => 8);
        $row = $this->jqGrid->get_data($req_param)->result_array();
        $count = count($row);
